...
void Configure(MonitorCore &core)
{
	...
	if (core.InitRegistryData(targetName, expiryDate)) {
		...
		std::wcout << L"   (请将 C:\\YourPath\\MonitorService.exe 替换为实际路径)\n"
				   << std::endl;

		std::wcout << L"2. **设置服务自动重启 (增强抗杀性):**" << std::endl;
		std::wcout << L"   sc failure MonitorSvc reset= 0 actions= restart/1000/restart/1000/restart/1000" << std::endl;
		std::wcout << L"   (服务失败后将立即重启)\n"
				   << std::endl;

		std::wcout << L"3. **启动服务 (常驻监控):**" << std::endl;
		std::wcout << L"   sc start MonitorSvc" << std::endl;
